...
#########################################################################
#       	This is GridMAT-MD, altered version for dcd usage      		#
#							#
#									#
...
open (INPUT2, $frame) or die "It seems \"$frame\" does not exist!\n";
@bilayer = (<INPUT2>);
close (INPUT2);

foreach my $line (@bilayer){

	chomp($line);
	my ($a_name, $r_name, $r_num, $x_coor, $y_coor, $z_coor) = split(' ', $line);
	$x_coor = $x_coor/10;
	$y_coor = $y_coor/10;
	$z_coor = $z_coor/10;
	my $new_line = ("$r_num $r_name $a_name $x_coor $y_coor $z_coor");
	push(@bilayer_fixed, $new_line."\n");

	# box vectors are given as argument to script
	# but config file must contain box_size vectors !
	$box_vectors = join(" ", split(",", $ARGV[2]));
...
foreach (@bilayer_fixed){
	my ($b_resid, $b_resname, $b_atomname, $bx, $by, $bz) = split(' ', $_);
...
if ($config{box_size} eq("solvent")){
	...
	foreach (@bilayer_fixed){
		my ($resid, $resname, $atomname, $x, $y, $z) = split(' ', $_);
...
if ($config{protein} eq("yes")){
	...
	foreach (@bilayer_protein){
		my ($resid, $resname, $atomname, $px, $py, $pz) = split(' ', $_);
...

{
	...
	open(OUTFILE2, ">", "temp_bottom_pbc.dat");
...
if ($config{average_pbc} eq("yes")){
	open(OUTFILE3, ">", "temp_average_pbc.dat");
...
if ($config{top_area} eq("yes")){
	open(OUTFILE4, ">", "temp_top_area.dat");
...
if ($config{bottom_area} eq("yes")){
	open(OUTFILE5, ">", "temp_bottom_area.dat");
